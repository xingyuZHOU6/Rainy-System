 ...
 * 原始路径：Assets\NOT_Lonely\BatchShaderSwapper\BatchShaderSwapper.cs
 * 分类/优先级：材质迁移工具 / C-扩展
 * 文件职责：编辑器批量查找并替换 Material Shader，默认可把材质迁移到 Weatherade Snow Coverage。
 * 数据流位置：选中/场景/项目材质 -> Shader 映射 -> 批量替换。
 * 主要 Unity 技术：EditorWindow、AssetDatabase、Material/Shader 枚举
 * 建议关注：非运行时脚印算法；研究现有材质如何批量接入雪 Shader 时阅读。
