 ...
 * 原始路径：Assets\NOT_Lonely\Weatherade SRS\Scripts\SRS_TerrainUtility.cs
 * 分类/优先级：Terrain 雪支持 / B-支持
 * 文件职责：烘焙 Terrain 远景 Albedo/Normal，并扩大 patch bounds 避免位移剔除。
 * 数据流位置：Terrain 近景多层材质 -> 远景 LOD 纹理。
 * 主要 Unity 技术：Terrain、离屏 Camera、ReadPixels、AssetDatabase
 * 建议关注：关注 Bake、CreateCam、SaveTex、patchBoundsMultiplier。
...
namespace NOT_Lonely.Weatherade
{
    ...
    public class SRS_TerrainUtility : MonoBehaviour
    {
        ...
        // 【中文注释】位移会把顶点推出原 patch AABB；扩大 bounds 防止可见雪面被错误剔除。
        ...
        // 【中文注释】临时切换 Bake Shader，用顶视相机生成 Terrain 远景 Albedo/Normal。
