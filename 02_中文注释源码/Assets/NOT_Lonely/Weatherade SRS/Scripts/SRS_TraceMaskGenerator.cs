 ...
 * 原始路径：Assets\NOT_Lonely\Weatherade SRS\Scripts\SRS_TraceMaskGenerator.cs
 * 分类/优先级：动态雪痕核心 / A-必读
 * 文件职责：每帧用相反方向的两台正交相机捕获雪面与交互物深度，生成有历史的痕迹高度/法线纹理。
 * 数据流位置：surfaceDepthRT + traceObjsRT + 上一帧 traceMaskRT -> 模糊 -> 细节 -> RGBA traceTex -> _SRS_TraceTex。
 * 主要 Unity 技术：多 Camera.Render、RenderTexture 历史反馈、Graphics.Blit、多 Pass Shader、世界锁定 UV、协程
 * 建议关注：按 CreateStuff、SetupCam、SetVariables、SimOneStep 阅读；理解每张 RT 的格式和生命周期。
...
namespace NOT_Lonely.Weatherade
{
    ...
    public class SRS_TraceMaskGenerator : MonoBehaviour
    {
        ...
        // 【中文注释】绑定所有 RT，并把最终 traceTex 发布为全局 _SRS_TraceTex。
        ...
        // 【中文注释】雪面深度用 maskRes/2，动态物与最终痕迹用全分辨率，是性能与细节的折中。
        ...
        // 【中文注释】surface 相机向下拍雪面；object 相机移到底部并旋转 180 度向上拍代理，两侧深度差近似压入厚度。
        ...
        // 【中文注释】覆盖体积跳格时反向滚动历史 UV，使脚印尽量锁定在世界空间。
        ...
        // 【中文注释】每帧：两相机 Render -> 接触+历史 -> R 模糊 -> 细节高度 -> 法线/通道打包。
