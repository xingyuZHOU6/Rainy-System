 ...
 * 原始路径：Assets\NOT_Lonely\Weatherade SRS\Scripts\CoverageBase.cs
 * 分类/优先级：积雪覆盖基础设施 / A-必读
 * 文件职责：覆盖系统基类：创建跟随目标的顶视正交深度相机、RenderTexture、指数深度矩和材质同步。
 * 数据流位置：场景深度 -> 指数一/二阶矩 -> 两遍模糊 -> 全局 _SRS_depth。
 * 主要 Unity 技术：Camera.Render、RenderTexture、Graphics.Blit、Shader 全局属性、协程、事件、URP AdditionalCameraData
 * 建议关注：重点读 UpdateSRSPosition、InitTexturesAndCam、GetDepth、ConvertDepthToVSM 和材质 Override 规则。
...
#pragma warning restore 0414
        ...
        // 【中文注释】按世界纹素大小对 X/Z 取整，避免跟随玩家时深度/痕迹映射持续亚像素漂移。
        ...
        // 【中文注释】创建原始 Depth、RG 指数矩和可选 RGBA 纹理，并让隐藏正交相机使用 SRS 专用 URP Renderer。
        ...
        // 【中文注释】环境遮挡链入口：Camera.Render -> 深度矩转换/模糊 -> 发布全局 _SRS_depth。
        ...
        // 【中文注释】指数化深度为一、二阶矩并做 RG 横/纵模糊；Shader 用方差与切比雪夫上界恢复遮挡概率。
        ...
        // 【中文注释】仅当 XOverride=0 才写全局值；Override=1 的 Terrain 材质可独立开启痕迹、位移和细分。
