 ...
 * 原始路径：Assets\NOT_Lonely\Weatherade SRS\Scripts\Extra\NL_Utilities.cs
 * 分类/优先级：共享工具 / B-支持
 * 文件职责：封装对象搜索、RT 创建/更新、噪声纹理、曲线烘焙、渲染管线判断和编辑器辅助。
 * 数据流位置：被 Coverage、Trace、Particle、Terrain 等多个系统调用。
 * 主要 Unity 技术：RenderTexture 工厂、Texture2D CPU 写入、AnimationCurve 烘焙、版本兼容
 * 建议关注：优先看 UpdateOrCreateRT；并留意未条件保护的 UnityEditor using。
