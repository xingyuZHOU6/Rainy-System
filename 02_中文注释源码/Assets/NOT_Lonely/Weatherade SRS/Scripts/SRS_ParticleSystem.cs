 ...
 * 原始路径：Assets\NOT_Lonely\Weatherade SRS\Scripts\SRS_ParticleSystem.cs
 * 分类/优先级：可选 GPU 飘雪 / C-扩展
 * 文件职责：以 RenderTexture 保存粒子状态、GPU 模拟并渲染飘雪。
 * 数据流位置：状态 RT ping-pong -> 顶点纹理读取 -> 雪花四边形。
 * 主要 Unity 技术：GPU 粒子、双缓冲 RT、Mesh、相机深度碰撞
 * 建议关注：邻接视觉系统，不生成地面压痕。
