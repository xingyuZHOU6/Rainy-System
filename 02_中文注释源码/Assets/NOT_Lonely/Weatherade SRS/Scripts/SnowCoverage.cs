 ...
 * 原始路径：Assets\NOT_Lonely\Weatherade SRS\Scripts\SnowCoverage.cs
 * 分类/优先级：雪控制器 / A-必读
 * 文件职责：雪的参数总控；把 Inspector 字段、材质 keyword 和纹理同步到所有 Weatherade 雪材质。
 * 数据流位置：CoverageBase 的环境遮挡数据 + 雪参数 -> Mesh/Terrain 雪材质。
 * 主要 Unity 技术：MonoBehaviour 序列化、LocalKeyword、全局 Shader 参数、材质覆盖、Shader 热切换
 * 建议关注：关注 Traces、Displacement、Tessellation、XOverride 优先级和普通/细分 Shader 切换。
...
#pragma warning restore 0414
        ...
        // 【中文注释】控制器只同步 Mesh/Terrain 的普通与细分四类 Weatherade Shader。
        ...
        // 【中文注释】Inspector 到 GPU 的总桥梁：keyword 决定编译分支，Material/Global 属性提供参数。
        public override void UpdateCoverageMaterial(Material material)
        {
            ...
            // 【中文注释】细分不是单纯 bool：编辑器会直接切换普通 Shader 与 Tessellation Shader。
