...
// 原始路径：Assets\NOT_Lonely\Weatherade SRS\Shaders\Blit\TraceMaskGen.shader
// 分类/优先级：动态雪痕核心 / A-必读
// 文件职责：三个全屏 Pass：深度接触+历史、噪声/边缘高度、有限差分法线与 RGBA 打包。
// 数据流位置：两张 Depth + 历史 R16 -> finalTraceMask -> RGBA _SRS_TraceTex。
// 主要 Unity 技术：全屏 Blit、深度比较、历史反馈、Simplex Noise、有限差分法线
// 建议关注：逐 Pass 阅读；牢记 RG=法线、B=基础痕迹、A=高度。
...
Shader "Hidden/NOT_Lonely/Weatherade/NL_TraceMaskGen"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            // 【中文注释】Pass 0：深度差 + 历史 + 衰减 + 区域边界，输出基础 R16 痕迹。
            Name "Primary Mask"
            CGPROGRAM
            #pragma vertex Vert
        ...
        Pass
        {
            // 【中文注释】Pass 1：生成凹陷、隆边和噪声；未压雪高度基准约 0.5。
            Name "Detail Mask"
            CGPROGRAM
            #pragma vertex Vert
            #pragma fragment Frag
        ...
        Pass
        {
            ...
            // 【中文注释】Pass 2：有限差分法线；输出 RG 法线、B 基础接触、A 最终高度。
            Name "Composite Mask"
            CGPROGRAM
            #pragma vertex Vert
            #pragma fragment Frag
