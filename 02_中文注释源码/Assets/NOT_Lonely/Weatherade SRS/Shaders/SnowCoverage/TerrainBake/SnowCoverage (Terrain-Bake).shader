...
// 原始路径：Assets\NOT_Lonely\Weatherade SRS\Shaders\SnowCoverage\TerrainBake\SnowCoverage (Terrain-Bake).shader
// 分类/优先级：Terrain 雪 Shader / B-支持
// 文件职责：Terrain 第一层/Add Pass、Tessellation 或远景烘焙入口。
// 数据流位置：Terrain splat 层 -> 雪覆盖/痕迹/位移 -> URP。
// 主要 Unity 技术：URP Terrain Lit、Add Pass、Basemap、Tessellation
// 建议关注：沿 include 进入 TerrainLitInput/Passes。
...
Shader "Hidden/NOT_Lonely/Weatherade/Snow Coverage (Terrain-Bake)"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            #pragma shader_feature_local _TERRAIN_INSTANCED_PERPIXEL_NORMAL //SRS: add this keyword to the DepthOnly pass to make the shader render the depth with the correct displacement
