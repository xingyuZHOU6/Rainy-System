...
// 原始路径：Assets\NOT_Lonely\Weatherade SRS\Shaders\SnowCoverage\MeshRender\SnowCoverage (Tessellation).shader
// 分类/优先级：Mesh 雪 Shader 入口 / A-必读
// 文件职责：启用 Hull/Domain 的 Mesh 雪 Shader，用细分支撑清晰几何脚印。
// 数据流位置：材质 -> Tessellation -> Snow Displace -> URP 各 Pass。
// 主要 Unity 技术：Shader Model 4.6、Tessellation、跨 Pass 位移
// 建议关注：与非细分版本 diff 阅读。
