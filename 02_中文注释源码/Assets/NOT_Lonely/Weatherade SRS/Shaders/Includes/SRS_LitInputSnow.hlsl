...
// 原始路径：Assets\NOT_Lonely\Weatherade SRS\Shaders\Includes\SRS_LitInputSnow.hlsl
// 分类/优先级：URP 雪材质接口 / B-支持
// 文件职责：声明雪 Mesh Shader 的材质 CBUFFER、纹理、采样器与 SurfaceData 初始化。
// 数据流位置：Material Properties -> URP Lit Pass。
// 主要 Unity 技术：SRP Batcher CBUFFER、URP SurfaceData
// 建议关注：对照 Shader Properties 查属性与 Override。
...
TEXTURE2D(_SRS_TraceTex);      //SAMPLER(sampler_SRS_TraceTex);
