...
// 原始路径：Assets\NOT_Lonely\Weatherade SRS\Shaders\Includes\SRS_LitGBufferPass.hlsl
// 分类/优先级：URP 多 Pass / B-支持
// 文件职责：URP Deferred/GBuffer 版本的雪表面入口。
// 数据流位置：雪表面数据 -> GBuffer。
// 主要 Unity 技术：URP Deferred、FragmentOutput、Mixed Lighting
// 建议关注：比较 Forward 与 GBuffer 的共同雪计算和不同输出。
