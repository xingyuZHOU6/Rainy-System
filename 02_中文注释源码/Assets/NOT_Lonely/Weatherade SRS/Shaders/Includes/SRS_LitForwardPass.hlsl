...
// 原始路径：Assets\NOT_Lonely\Weatherade SRS\Shaders\Includes\SRS_LitForwardPass.hlsl
// 分类/优先级：URP 多 Pass / B-支持
// 文件职责：URP Forward 入口，把 SnowCoverage 结果接到 PBR 光照。
// 数据流位置：顶点位移 -> Varyings -> 雪表面修改 -> UniversalFragmentPBR。
// 主要 Unity 技术：URP Forward、InputData、GI、雾、阴影
// 建议关注：跟踪 LitPassVertex 与 LitPassFragment。
