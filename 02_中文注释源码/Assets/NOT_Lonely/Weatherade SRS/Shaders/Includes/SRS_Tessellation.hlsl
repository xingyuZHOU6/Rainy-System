...
// 原始路径：Assets\NOT_Lonely\Weatherade SRS\Shaders\Includes\SRS_Tessellation.hlsl
// 分类/优先级：雪几何位移 / A-必读
// 文件职责：曲面细分控制、边因子、Hull/Domain 程序，并在细分后调用雪位移。
// 数据流位置：原三角形 -> 动态细分 -> Displace -> 最终顶点。
// 主要 Unity 技术：DX11 Tessellation、Hull、Domain、视锥剔除、距离/雪量自适应
// 建议关注：结合 Tessellation Shader 读 PatchConstantFunction 和 DomainProgram。
...
// 【中文注释】每 Patch 计算边/内部细分因子，可按距离、边长和雪/痕迹 mask 自适应。
...
[domain("tri")]
// 【中文注释】重心插值细分后顶点，再进入对应 Pass；位移发生在更密的顶点上。
