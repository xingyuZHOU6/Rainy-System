...
// 原始路径：Assets\NOT_Lonely\Weatherade SRS\Shaders\Includes\SRS_LightingCommon.hlsl
// 分类/优先级：雪光照 / C-扩展
// 文件职责：雪的闪光、近似次表面散射和颜色增强。
// 数据流位置：雪 mask + URP 灯光/GI -> 最终雪光照。
// 主要 Unity 技术：URP PBR、附加灯、屏幕空间闪光、SSS
// 建议关注：不影响脚印生成，理解外观时再读。
