...
// 原始路径：Assets\NOT_Lonely\Weatherade SRS\Shaders\Includes\SRS_CoverageCommon.hlsl
// 分类/优先级：积雪遮挡算法 / A-必读
// 文件职责：切比雪夫上界、深度矩遮挡、方向/体积边界、绘制遮罩、随机采样和法线混合。
// 数据流位置：_SRS_depth 与世界坐标 -> 基础积雪可见度。
// 主要 Unity 技术：HLSL 导数、VSM/EVSM、随机纹理采样、RNM
// 建议关注：重点读 ComputeBasicAreaMask、DirMask、CalcVolumeMask、PaintMaskRGBA。
...
// 【中文注释】把当前点深度做同样指数变换，与模糊矩比较得到屋檐遮挡 mask。
...
// 【中文注释】0.5 为添加/擦除分界；RGBA 同时可承载覆盖与多层雪权重。
