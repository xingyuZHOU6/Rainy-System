...
// 原始路径：Assets\NOT_Lonely\Weatherade SRS\Shaders\Includes\SRS_Triplanar.hlsl
// 分类/优先级：雪纹理采样 / B-支持
// 文件职责：世界空间三平面投影及软混合/法线转换。
// 数据流位置：世界位置+世界法线 -> 无模型 UV 依赖的雪纹理采样。
// 主要 Unity 技术：Triplanar、纹理导数、LOD、法线变换
// 建议关注：理解 triMask、轴向 UV 和法线回到世界空间。
