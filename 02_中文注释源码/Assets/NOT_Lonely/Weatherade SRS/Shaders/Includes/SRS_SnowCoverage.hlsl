...
// 原始路径：Assets\NOT_Lonely\Weatherade SRS\Shaders\Includes\SRS_SnowCoverage.hlsl
// 分类/优先级：雪着色与位移核心 / A-必读
// 文件职责：把遮挡、方向、雪纹理、绘制遮罩和动态痕迹合成为 PBR 表面与顶点位移。
// 数据流位置：_SRS_depth + _SRS_TraceTex + 雪纹理 -> albedo/normal/smoothness/位移。
// 主要 Unity 技术：世界空间三平面、材质混合、RNM、顶点位移、跨 Pass 一致性
// 建议关注：先读 SnowCoverage 的 _TRACES_ON，再读 Displace 的 _TRACES_ON。
...
// 【中文注释】像素核心：组合遮挡、方向、雪纹理和痕迹，改写 PBR 表面数据。
...
half3 geomNormalWS, inout half finalSnowMask, float4 vertexColor, out half sssMask, out half highlightBrightness, float2 uv = 0, half dither = 0)
{
    ...
    // 【中文注释】世界坐标进入覆盖相机裁剪空间，再用 _SRS_depth.RG 矩重建遮挡。
...
*/
    ...
    // 【中文注释】_SRS_TraceTex.RG 改法线，B/A 控制痕迹范围、颜色和露底；Displace 还会用 A 压低几何。
    #ifdef _TRACES_ON
...
///Snow displacenent///
///////////////////////
// 【中文注释】顶点核心：基础雪抬高，痕迹 A 通道压低；清晰轮廓需要高密网格或 Tessellation。
...
// 【中文注释】各 URP Pass 通过此入口获得已位移顶点，保持颜色、深度和阴影一致。
