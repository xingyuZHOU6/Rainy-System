...
// 原始路径：Assets\NOT_Lonely\Weatherade SRS\Shaders\CGIncludes\SRS_CoverageCommon.cginc
// 分类/优先级：动态痕迹直接依赖 / A-必读
// 文件职责：TraceMaskGen 直接 include 的 Built-in 风格通用覆盖函数，提供边界 mask 等计算。
// 数据流位置：TraceMaskGen UV -> CalcBorderMask。
// 主要 Unity 技术：UnityCG、通用遮罩数学
// 建议关注：虽然位于 CGIncludes，它是当前 URP 痕迹 Blit 的直接依赖。
